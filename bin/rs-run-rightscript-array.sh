...
api_params="-d server_array[right_script_href]=https://my.rightscale.com/api/acct/$rs_api_account_id/right_scripts/$rightscript_id"
[[ $3 ]] && api_params="$api_params -d server_array[server_template_hrefs]=https://my.rightscale.com/api/acct/$rs_api_account_id/ec2_server_templates/$3"

echo "Running RightScript [$rightscript_id] on Server Array [$server_array_id]."
echo "GET: $api_url"
curl -X POST -H X-API-VERSION:"$rs_api_version" -b "$rs_api_cookie" "$api_url" "$api_params"